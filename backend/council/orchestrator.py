"""
Orchestrator v2.1: Integrates SmartRouter + TaskDecomposer + SelfRefine + ToT + SharedMemory.
Flow: Load context -> Route -> Decompose (if complex) -> Protocol -> Refine -> Return
"""
...
from ..memory.shared_memory import load_shared_memory, SharedMemoryContext


class Orchestrator:
    """v2.1 orchestrator with shared memory context injection."""
    ...
    def __init__(self, config: CouncilConfig):
        ...
        self._shared_ctx: SharedMemoryContext | None = None
    ...
    def _get_shared_context(self) -> SharedMemoryContext:
        if self._shared_ctx is None:
            self._shared_ctx = load_shared_memory()
        return self._shared_ctx

    async def run(self, session: Session) -> SessionResult:
        """Full pipeline: load context -> route -> decompose -> protocol -> refine."""

        # Inject shared memory context into session
        shared = self._get_shared_context()
        context_block = shared.as_context_block()
        if context_block and not session.context:
            session.context = context_block
        elif context_block and session.context:
            session.context = f"{context_block}\n\n{session.context}"
    ...
    async def _fast_path(self, session: Session, routing: RoutingDecision) -> SessionResult:
        ...
        response = await self.router.query(model, messages, context=context)
